...
public class King extends Pieces{
    ...
    public boolean isChecked(int posY, int posX, Board board, boolean color){
        ...
        for(int i = 0;i<8;i++){
            if(!isInYDirection && (board.getBoard()[i][posX] instanceof Rook || board.getBoard()[i][posX] instanceof Queen) && board.getBoard()[i][posX].getIsBlack() != color){
                return true;
            } else if(board.getBoard()[i][posX] != null){
                    isInYDirection = true;
                }

            if(!isInXDirection && (board.getBoard()[posY][i] instanceof Rook || board.getBoard()[posY][i] instanceof Queen) && board.getBoard()[posY][i].getIsBlack() != color){
                return true;
            } else if(board.getBoard()[i][posX] != null){
                isInXDirection = true;
            }
